...
function exampleAutoCrop(){
	...
	imageAutoCrop.load("../images/cheetah.jpg", function(){
		...
		/*
		Marvin.scale(imageAutoCrop.clone(), imageAutoCrop, 300);
		var map = Marvin.moravec(imageAutoCrop.clone(), imageAutoCrop, 5, 20000);
		
		for(var x=0; x<map.length; x++){
			for(var y=0; y<map.length; y++){
				if(map[x][y] > 0){
					imageAutoCrop.fillRect(x,y,5,5,0xffff0000);
				}
			}
		}
		//imageAutoCrop.draw(document.getElementById("canvasAutoCrop"));
